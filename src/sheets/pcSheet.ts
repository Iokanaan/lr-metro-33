...
export const pcSheet = function(sheet: Sheet): PcSheet {
    ...
    for(let i=0; i<stats.length; i++) {
        ...
        const stats_curr = _sheet.find(stats[i] + "_curr") as Component<string>
        _sheet.stats[stats[i]] = {
            "max": signal(stats_val.value()),
            "curr": signal(parseInt(stats_curr.value()))
        }
        stats_curr.on("update", stringToIntegerUpdateHandler(_sheet.stats[stats[i]].curr))
